...
- Tự triển khai **100% thủ công** 4 cấu trúc dữ liệu cốt lõi (Data Structures): 
  ...
  4. **Cây tiền tố (Trie)**
  5. **Hàng đợi ưu tiên (Max-Heap)**
- Ứng dụng các cấu trúc dữ liệu đã học để giải quyết bài toán thực tế: Tìm kiếm siêu tốc bằng ID, lọc sản phẩm theo khoảng giá, gợi ý tìm kiếm (Autocomplete), và hiển thị top sản phẩm đánh giá cao.
...
*   **`src/datastructure/`**: Nơi chứa lõi Cấu trúc dữ liệu "nhà làm":
    *   `/list/`: Cấu trúc Danh sách liên kết đơn (Thay thế hoàn toàn cho `ArrayList`).
    *   `/hash/`: Cấu trúc Node cho Hash Map (Xử lý va chạm).
    *   `/tree/`: Cấu trúc Node cho Cây nhị phân.
    *   `/trie/`: Cấu trúc Node cho Cây tiền tố.
