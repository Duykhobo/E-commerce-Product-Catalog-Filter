...
public class RatingSorter {
    ...
    // Heapify Up (chìm lên) dựa theo Rating
    ...
    // TODO (Nguyễn Thanh Duy): Viết hàm trích xuất sản phẩm có Rating cao nhất
    // (Extract Max) và Heapify Down
    public Product extractMax() {
        ...
        size--;
        heap[size] = null; //
        // 4. Gọi đệ quy hoặc vòng lặp để đưa phần tử gốc chìm xuống đúng vị trí
        // (Heapify Down)
        heapifyDown(0);
        return max;
    }
    ...
    private void heapifyDown(int i) {
        ...
        if (l < size && heap[l].getRating() > heap[largest].getRating()) {
            largest = l;
        }
        if (r < size && heap[r].getRating() > heap[largest].getRating()) {
            largest = r;
        }
